...
class DictDifferTests(unittest.TestCase):
    def test_without_dot_notation(self):
        (change1,) = diff({"a": {"x": 1}}, {"a": {"x": 2}}, dot_notation=False)

        assert change1 == ("change", ["a", "x"], (1, 2))

    def test_with_dot_notation(self):
        (change1,) = diff({"a": {"x": 1}}, {"a": {"x": 2}})
    ...
    def test_path_limit_as_list(self):
        ...
        res = [("add", "", [("author", {"first_name": "John", "last_name": "Doe"})])]
    ...
    def test_path_limit_deletion(self):
        ...
        res = [("remove", "", [("author", {"first_name": "John", "last_name": "Doe"})])]
    ...
    def test_path_limit_change(self):
        ...
        res = [
            (
                "change",
                ["author"],
                ({"first_name": "John", "last_name": "Do"}, {"first_name": "John", "last_name": "Doe"}),
            )
        ]
    ...
    def test_expand_addition(self):
        ...
        res = [("add", "", [("foo", "bar")]), ("add", "", [("apple", "banana")])]
    ...
    def test_expand_deletion(self):
        ...
        second = {}
        diffed = list(diff(first, second, expand=True))
        res = [("remove", "", [("foo", "bar")]), ("remove", "", [("apple", "banana")])]
    ...
    def test_ignore_with_unicode_sub_keys(self):
        ...
        assert len(list(diff(first, second, ignore=[[u"a", u"aא", u"aa"]]))) == 0
    ...
    def test_ignore_with_ignorecase(self):
        ...
        assert list(diff({"a": 1, "b": 2}, {"A": 3, "b": 4}, ignore=IgnoreCase("a"))) == [("change", "b", (2, 4))]
    ...
    def test_complex_diff(self):
        ...
        d1 = {
            ...
            "float_here": 0.454545,
            "complex": [{"id": 83865, "goal": Decimal("2.000000"), "state": u"active"}],
            "profile_id": None,
            "state": u"active",
        }
...
class DiffPatcherTests(unittest.TestCase):
    ...
    def test_remove_allow_missing_keys(self):
        first = {"a": "b"}
        assert first == patch([("remove", "", [("d", "c")])], first, allow_missing_keys=True)

    def test_remove_forbid_missing_keys(self):
        first = {"a": "b"}
        self.assertRaises(KeyError, patch, [("remove", "", [("d", "c")])], first)
    ...
    def test_remove_set(self):
        first = {"a": set([1, 2, 3])}
        second = {"a": set([1])}
        assert second == patch([("remove", "a", [(0, set([2, 3]))])], first)

    def test_add_set(self):
        first = {"a": set([1])}
        second = {"a": set([1, 2])}
        assert second == patch([("add", "a", [(0, set([2]))])], first)
...
def test_ignore_dotted_ignore_key(ignore, dot_notation, diff_size):
    ...
    config_dict = OrderedDict([("address", "devops011-slv-01.gvs.ggn"), (key_to_ignore, "3 secs")])

    ref_dict = OrderedDict([("address", "devops011-slv-01.gvs.ggn"), (key_to_ignore, "4 secs")])

    assert diff_size == len(list(diff(config_dict, ref_dict, dot_notation=dot_notation, ignore=[ignore])))
